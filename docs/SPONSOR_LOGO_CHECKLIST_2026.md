# 2026 Sponsor Logo Checklist

Updated from the confirmed sponsor register on 18 September 2026.

## Logo files still required

- Everest Home Loans
- Solve Education Consultancy
- Sunrise Energy
- APBiz Solutions
- Colorate Prints
- Elite Curtains and Blinds
- RS Painting
- Capkon Home Loans
- Melbourne Institute of Technology
- Preferred Education & Migration Consultants
- Central Australian College

Until approved artwork is supplied, the visitor app displays the sponsor's name in a branded placeholder. Add final PNG or SVG artwork to `frontend/public/brand/sponsors/`, update the matching row in `frontend/src/content/seed/data.ts`, and verify the logo on both white and gold sponsor tiles.

## Register decisions needing confirmation

- Student Destiny and Khukuri Melbourne are not published because the register says "Not approved yet".
- Everest Landscaping appears under both Gold and Silver; it remains Gold because the earlier approved record and existing artwork identify it as Gold.
- Preferred Consultancy appears once with a $1,000 paid note and again as an approved Silver sponsor; the explicit Silver category is used.
- Capkon Home Loans and Central Australian College are confirmed Silver sponsors.
